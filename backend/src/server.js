...
// Database connection manager with connection caching
  ...
  socket.on("send_message", async (data) => {
    try {
      ...
      const populatedMessage = await Message.findById(newMessage._id)
        .populate("senderId", "name profileImg role")
        .lean();

      const populatedConversation = await Conversation.findById(conversationId)
        .populate("participants", "name profileImg role")
        .populate("listingId", "title photos location")
        .lean();

      // Emit notification badge to all other participants
      conversation.participants.forEach((pId) => {
        if (pId.toString() !== senderId.toString()) {
          io.to(`user_${pId.toString()}`).emit("new_message_notification", {
            conversationId,
